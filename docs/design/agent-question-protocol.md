...
- **Analyst:** When feedback is too vague, return `open_questions` instead of creating tasks. Do not create a ticket until questions are answered. You may ask multiple questions when clarification would help; prefer tradeoff questions when appropriate (options A/B/C with pros and cons).
